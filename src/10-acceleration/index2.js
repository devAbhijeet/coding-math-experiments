...
let render = () => {
  ...
  const { thrusting, turningLeft, turningRight } = controls;
  // angle += turningLeft ? -0.05 : turningRight ? 0.05 : angle;
  if (turningLeft) {
    angle -= 0.05;
  }
  if (turningRight) {
  ...
  ship.acceleration.setAngle(angle);
  ship.acceleration.setLength(thrusting ? 0.01 : 0);
...
let handleKeyEvent = (e) => {
  if (e) {
    ...
    switch (keyCode) {
      case 38:
        controls.thrusting = isKeyDown ? true : false;
        break;
      case 39:
        controls.turningRight = isKeyDown ? true : false;
        break;
      case 37:
        controls.turningLeft = isKeyDown ? true : false;
        break;
      default:
      //
    }
